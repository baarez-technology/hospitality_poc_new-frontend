...
// Room type options - matches database room_types table
export const ROOM_TYPES = [
  { value: 'Minimalist Studio', label: 'Minimalist Studio', price: 180 },
  { value: 'Coastal Retreat', label: 'Coastal Retreat', price: 185 },
...
// Calculate nights between dates (minimum 1 night for same-day bookings)
export function calculateNights(checkIn, checkOut) {
  if (!checkIn || !checkOut) return 0;
  const start = new Date(checkIn);
  const end = new Date(checkOut);
  const diff = Math.floor((end - start) / (1000 * 60 * 60 * 24));
  // Return at least 1 night for same-day bookings
  return Math.max(1, diff);
}

// SST: Import centralized pricing
import { calculateRoomPrice, getTaxRate } from '@/utils/pricing-sst';

// GST slab lookup - DELEGATES TO SST
function getRoomGSTRate(pricePerNight: number): number {
  return getTaxRate(pricePerNight).rate;
}

// Calculate booking amount - DELEGATES TO SST
export function calculateBookingAmount(roomType, nights, taxRate?: number) {
  ...
  // SST: Use centralized pricing function
  const pricing = calculateRoomPrice(baseRate, nights);
